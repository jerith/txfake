...
from txfake.fake_connection import (
...
class TestFakeHttpServer(TestCase):
    ...
    if sys.version_info[0] == 3:
        skip = "Agent doesn't set the Host header correctly on py3."
    ...
    def test_POST_request(self):
        ...
        agent = fake_http.get_agent()
        self.assertTrue(IAgent.providedBy(agent))
        response = yield agent.request("POST", b"http://example.com/hello")
        # We got a valid request and returned a valid response.
        [request] = requests
    ...
    def test_PUT_request(self):
        ...
        agent = fake_http.get_agent()
        self.assertTrue(IAgent.providedBy(agent))
        response = yield agent.request("PUT", b"http://example.com/hello")
        # We got a valid request and returned a valid response.
        [request] = requests
